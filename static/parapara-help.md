...
## シンボルフォントを置換
`原文`のシンボルフォント由来の文字をシンボルフォント置換辞書に従って置換します。<br>
...
## 著作権配慮の構造ファイルを出力
共同作業用に、**原文/訳文を含まない**「構造ファイル」を出力します。

- 出力先: `data/structure/(PDF名).structure.json`
- 出力時に除外される項目: `src_html`, `src_text`（`src_trxt`）, `src_joined`, `src_replaced`, `trans_auto`, `trans_text`

このファイルは、段落の `order` / `block_tag` / `group_id` / `join` など「文書の構造調整」を共有する用途を想定しています。

## 構造ファイルを取り込み
出力した「構造ファイル」を取り込み、既存のブックデータ（json）へ反映します。

- 取り込み前にバックアップを作成: `data/backup/` に元の `(PDF名).json` を複写します
- 取り込み時に更新しない項目: `src_html`, `src_text`（`src_trxt`）, `src_joined`, `src_replaced`, `trans_auto`, `trans_text`
- `join` が変更された場合、連結文などの整合性のため `src_joined/src_replaced` を再構築します（派生項目が更新され得ます）
...
## 全文検索パネル
検索語を入力すると、各段落の「原文」「訳文」「自動翻訳」から該当する段落を表示します。
検索結果をクリックすると該当段落へジャンプし、該当語がハイライトされます。
