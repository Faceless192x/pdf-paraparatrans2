...
**自動翻訳時の例外（下訳/draft 扱いにする段落）**

翻訳APIの無駄打ちや崩れを避けるため、`置換文(src_replaced)` が次の条件に当てはまる段落は、
自動翻訳の実行時に **下訳（draft）** に設定され、翻訳APIへは送信しません（ステータスが `未翻訳(none)` の段落のみ対象）。

- 空（空白のみ含む）
- 数字と記号のみ（空白は無視）
- 英字が2文字以下（例: `HP`, `GM` など）

※ 既に `自動翻訳(auto)` になっている段落でも、`Auto/訳文` が `置換文` のコピーに見える場合は、同条件で **下訳（draft）へ自動補正** されます（実訳を壊さないための安全策です）。
