...
- （任意）`src_html` から `原文(src_text)` を再生成し、シンボルフォント（例: Wingdings）の文字を `data/symbolfont_dict.txt` で置換できる
...
| - | (原文再生成) | `src_html` から `原文(src_text)` を作り直し、シンボルフォント由来の文字を `data/symbolfont_dict.txt` に従って置換します。 |
