...
**英文PDFを、段落単位で“読みやすく・直しやすく”翻訳するためのローカルWebアプリ**です。PDFを見ながら段落を整え、APIで翻訳し、原文と訳文を左右で突き合わせて推敲できます。
...
- ただし翻訳は、選んだ翻訳サービス（Google / DeepL）へ **文章が送信** されます（PDFそのものを送るわけではありません）。Ollama を使う場合は完全にローカルで翻訳され、外部送信はありません。
- 翻訳APIは有料枠があるため、使いすぎると費用が発生する場合があります（Ollama は無料）。

**まずは3分で試す**
1. 翻訳サービスを1つ選ぶ（迷ったらまずはGoogle、APIキー不要ならOllama）
    - [Google 翻訳 API（APIキー方式）](SETUP_GOOGLE_TRANSLATE.md)
    - [DeepL API](SETUP_DEEPL.md)
    - [Ollama（ローカル LLM・APIキー不要）](SETUP_OLLAMA.md)
2. このアプリをPCに用意する（おすすめ: zipでダウンロード）
    - 下の「ローカル環境（おすすめ）」へ
3. 起動したらブラウザで `http://localhost:5077/` を開く
4. PDFを用意（どちらか）
    - A) 一覧画面の **「PDFを追加」** からアップロード
    - B) `data/` にPDFを入れる
5. PDF名をクリック → **1.パラグラフ抽出**
    - 見出し/ヘッダ/フッタを整えたい → **(自動タグ付け)**
    - 段落が途中で切れている、前後がおかしい → 段落順を入れ替えて **2.段落連結**
    - 用語を固定したい → **3.全対訳置換**（`config/dict.txt`）
    - 最後に → **4.全翻訳**

**できること**
- Google 翻訳 API / DeepL API / Ollama（ローカル LLM）に対応
- PDFと抽出段落を同一画面で並べ、選択段落がPDF上でハイライト（段落の並び替えを高速に）
- 段落ごとに翻訳ステータス管理（未翻訳/自動翻訳/下訳/確定）で推敲が進めやすい
- 対訳辞書で“翻訳前に置換”して用語揺れを抑制（ゲーム用語などに強い）
- 文書ごとの辞書を作成でき、文書単位で使用する辞書を選択できる（標準辞書と複数選択可）
...
- URLから文書データを作成し、PDFと同じように翻訳の管理ができる
- 原文や訳文を構成した段落通りに `Markdown (.md)` または `Text (.txt)` で出力できる
...
- `データ入出力` メニューから **「構造出力」**: `data/structure/<PDF名>.structure.json` を生成し、ダウンロードします。
- `データ入出力` メニューから **「構造取込」**: 構造ファイルを選択して取り込みます（保護のため適用前のJSONが `data/backup/` に複写されます）
...
- 翻訳APIのキー（Google / DeepL のどちらか）、または Ollama（APIキー不要）
- 翻訳したいPDF（`data` フォルダに入れる、または一覧画面からアップロード）
- 用語辞書（`config` フォルダに入れる）

## 表示項目（列）とステータス（詳細）

| No | 項目 | 概要 |
| --- | --- | --- |
| 1 | HTML | 原文のフォント名やフォントサイズをStyleタグとして持つ文字列。通常は使いません。 |
| 2 | 原文 | 英文テキスト |
| 3 | 置換文 | 対訳辞書で置換したテキスト。 このテキストが翻訳APIの対象になります。 |
...
| 自動翻訳 | 対象外 | 自動翻訳後に未翻訳から自動翻訳に変わります |
| 下訳 | 対象外 | 推敲中のパラグラフに使用します。 |
| 確定 | 対象外 | 翻訳を確定したパラグラフに使用します。 |

**自動翻訳時の例外**

翻訳APIの無駄打ちや崩れを避けるため、`置換文` が次の条件に当てはまる段落は、
自動翻訳の実行時に **下訳** に設定され、翻訳APIへは送信しません。

- 空（空白のみ含む）
- 数字と記号のみ（空白は無視）
- 英字が2文字以下（例: `HP`, `GM` など）

※ 既に `自動翻訳` になっている段落でも、**上の条件（空/数字と記号のみ/英字2文字以下）に当てはまり**、かつ `Auto/訳文` が `置換文` のコピーに見える場合に限って、**下訳へ自動補正** されます（実訳を壊さないための安全策です）。
...
7. （どちらか）翻訳したいPDFを用意する
    - A) `data` フォルダを作成し、PDFを入れる（おすすめ）
    - B) 起動後、一覧画面の **「PDFを追加」** からPDFをアップロード
8. 起動する
    ```
    python pdf-paraparatrans.py
    ```
    > これはWebサーバーを起動するコマンドなので、**実行したまま終了しません（待ち受け状態になります）**。
    > 停止したいときは、このターミナルで **Ctrl + C** を押してください。
9. ブラウザで `http://localhost:5077/` を開く（PDF一覧が出ます）
10. Pdf-paraParaTransは Webサーバーを起動するので、**実行したまま終了しません（待ち受け状態になります）**。
...
1. GitHubのリポジトリ画面 [https://github.com/runequest77/pdf-paraparatrans2] で **Code** → **Codespaces** → **Create codespace** を選択
2. 依存関係のインストール
    ...
    - もし入っていない/失敗した場合は、ターミナルで以下を実行してください
        ...
        python -m pip install -r requirements.txt
        ```
3. （推奨）GitHub 側で **Codespaces Secrets（User secrets）** を登録（envにAPIキーを書かない）
...
4. （どちらか）翻訳したいPDFを用意する
    - A) VS Code（Web）のファイルツリーから `data/` にPDFをアップロード
    - B) 起動後、一覧画面の **「PDFを追加」** からPDFをアップロード（おすすめ）
    - `data` は通常初回セットアップ時に作成されます（無ければ作成してください）
5. 起動する（ターミナル）
    ```
    python pdf-paraparatrans.py
    ```
...
### 翻訳APIの準備
 ...
 [Ollama（ローカル LLM）のセットアップ（APIキー不要）](SETUP_OLLAMA.md)

どれを選べばいいか迷ったら、まずは **Google（APIキー方式）** がおすすめです。APIキーの取得が面倒な場合は **Ollama** を試してください。

### 🔠 置換辞書の準備
辞書編集の詳しい手順は、次のガイドを参照してください。

- [docs/DICTIONARY_EDITING.md](docs/DICTIONARY_EDITING.md)

共通辞書は `config/dict.txt`（タブ区切り）で管理し、編集後に **「3.全対訳置換」** または **「ページ対訳置換」** を実行して反映します。

### 🔣 シンボルフォント置換の準備
シンボルフォント編集の詳しい手順は、次のガイドを参照してください。

- [docs/SYMBOLFONT_EDITING.md](docs/SYMBOLFONT_EDITING.md)

置換ルールは `config/symbolfont_dict.txt` と `config/symbolfonts.txt` で管理し、編集後に **「(シンボル置換)」** を実行して反映します。

### 🅿️ PDF一覧画面
- `data`フォルダにあるPDF/フォルダが一覧表示されます。PDF名をクリックすると、そのPDF用の翻訳画面に移動します。
- フォルダ作成/名前変更/削除、PDFのフォルダ移動が可能です

#### PDFのアップロード（一覧画面から追加）
右上の **「PDFを追加」** からPDFをアップロードできます。アップロードが完了すると一覧が自動更新されます。

**仕様 / 注意点**
- アップロードできるのは `.pdf` のみです。
- 保存先は `data/` で、アップロードしたファイルは `data/<pdf_name>.pdf` として保存されます。
- **同名（拡張子を除いた名前）が既にある場合はエラー**になります。ファイル名を変えて再アップロードしてください。

### 🌐 URLブック

PDFではなくWebページを対象にした「URLブック」を作成できます。

使い方（作成・取込・クロール・ルール保存）と運用の注意点は、次のガイドを参照してください。

- [docs/URL_BOOK_GUIDE.md](docs/URL_BOOK_GUIDE.md)

ブラウザ拡張（Chrome/Edge）を使った取込手順も、上記ガイドに記載しています。
データ入出力ダイアログの **「ブラウザ拡張をダウンロード」** から、拡張ZIPを取得できます。
...
| 1 | パラグラフ抽出 | PDFを用意した後（一覧画面でアップロード、または `data/` に配置）、**最初に `1.パラグラフ抽出` をクリック**します。<br>OKを押すと、少し待ち時間があった後にパラグラフが表示されます。 |
| - | (自動タグ付け) | 見出しを推定して `block_tag`（h1〜h6等）をセットします。<br>あわせて段落連結候補（`join`）も自動でセットします。 |
| - | (シンボル置換) | シンボルフォント由来の文字を `config/symbolfont_dict.txt` に従って置換して`原文`にセットします。 |
| 2 | 段落連結 | `結合フラグ(join=1)` の段落を、同じタグの直前の段落（base: join=0）の `結合文` に連結して作り直します。<br>連結結果は **base(join=0) の段落だけ** が `結合文` に保持し、join=1 側の `結合文` は空になります。<br>※ ページ境界をまたいで連結できるのは `p` のみです（それ以外はページ内のみ）。<br>`結合文` が変化した段落は `置換文` を同値に更新し、`ステータス` を `none` に戻します（変化しない段落は維持）。|
| 3 | 全対訳置換 | 対訳辞書 `config/dict.txt` に従って`結合文`を置換して`置換文`にセットします。 |
...
| - | 全文検索パネル | 検索語を入力すると、各段落の「原文」「訳文」「自動翻訳」から該当する段落を検索します。<br>検索結果をクリックで該当段落へジャンプし、該当語がハイライトされます。|
| - | PDF | PDFの現在のページを表示します。<br>チェックボックスで表示/非表示を切り替えられます。 |
| - | ページ対訳置換 | 現在のページに対して対訳置換を実行します。|
| - | ページ翻訳 | 現在のページに対して自動翻訳を実行します。|
| - | 訳揃え | 同一 `結合文` の段落を対象に、より上位の `ステータス` を持つ訳へ `Auto/訳文/ステータス` を揃えます（文書全体が対象）。|
| - | Log | 処理ログをポップアップで確認できます。 |
| - | (辞書抽出) | 現在の文書から固有名詞と解釈しうる文字列を対訳辞書 `config/dict.txt` に「ステータス9（未翻訳）」で追加します。<br>すでに対訳辞書にある文字列やステータスは維持されます。 |
| - | 辞書登録 | 対訳辞書に単語を登録できます。 |
| - | データ入出力 | 構造ファイル（`.structure.json`）の出力/取込ができます。原文/訳文を含まないため、複数人で構造だけ共有したい場合に使えます。 |
...
4. **[原文]** / **[訳文]** / **[コメント]** は編集できます。<br>パラグラフ右の [保存] ボタンで内容が保存され、編集画面が閉じます。
5. ステータス
    ...
    - `訳揃え` は同一 `src_joined` 内で `fixed > draft > auto > none` の順に代表訳を選び、下位ステータス側を上書きして統一します。
6. **Alt++（または Alt+;）** で `結合フラグ(join)` のON/OFFができます。トグルすると周辺の `結合文/置換文` が再計算されます（影響範囲が広い場合は表示が再読み込みされることがあります）。
...
## パラグラフ自動解析について
パラグラフがどれぐらいきれいにまとまるか、段落の順序が適切かは、PDFの作りによります。
...
## 更新履歴
- v2.8.0: 2026/02/22 URLブックを追加。文書別辞書とメンテナンス機能を追加。
- v2.7.0: 2026/02/15 検索強化、フォルダ管理（作成/名前変更/削除/移動）を追加、Markdown/テキスト出力を追加。
- v2.6.0: 2026/01/29 全文検索追加。
- v2.5.0: 2026/01/27 段落結合を即時反映に改善
- v2.4.0: 2026/01/26 コメント/URLリンク追加。
- v2.1.0: 2026/01/25 セットアップ/ショートカットキーなどの微調整。
