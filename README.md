...
### 🔠 置換辞書の準備
`config\dict.txt` に翻訳前に置換したい単語を登録することができます。タブ区切りテキストファイルです。
...
自動的に置換されるわけではなく、「3.全対訳置換」もしくは「ページ対訳置換」を実行する必要があります。
...
PDFごとの辞書（`*.dict.txt`）を作成して、文書単位で使う辞書を選択できます。
- 共通辞書だけ使う
- 共通辞書 + 文書辞書を併用する
- 文書辞書だけ使う
...
2. 必要に応じて、次の方法で取り込み
    - URLを1ページずつたどる取り込み
    - HTML文字列の直接取り込み
    - 巡回取り込み（開始URLからリンクをたどる）

#### サイトルールの保存

取り込み対象を安定させたい場合、ドメインごとに抽出ルール（セレクタなど）を保存できます。
