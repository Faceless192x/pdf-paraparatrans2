...
### 🔠 置換辞書の準備
`config\dict.txt` に翻訳前に置換したい単語を登録します。タブ区切りテキストファイルです。
...
| - | (シンボル置換) | `src_html` から `原文(src_text)` を作り直し、シンボルフォント由来の文字を `config/symbolfont_dict.txt` に従って置換します。 |
...
| - | 訳揃え | 同一 `src_joined` の段落を対象に、より上位の `trans_status` を持つ訳へ `trans_auto/trans_text/trans_status` を揃えます（文書全体が対象）。|
