...
### フェーズ 6: エクスポート Blueprint の分離 ✅（本PRで実施）
...
## Copilot エージェントとの作業フロー

### フェーズごとにマージする

> **Q: リファクタリングは都度マージしたほうが良いですか？**

**はい、1 フェーズ＝1 PR を原則とし、必ず main にマージしてから次のフェーズに進んでください。**

理由:
- コンフリクトの範囲を最小化できる
- レビューが小さな単位で完結し、問題の特定が容易
- Copilot エージェントが前のフェーズの変更を base として正しく認識できる
- 万一問題が起きた場合に revert の影響が限定される

### 次のフェーズを依頼するメッセージ

> **Q: マージした後はメッセージを変えたほうがいいのか？**

**はい、マージ後は新しいメッセージで次のフェーズを依頼してください。**

前のフェーズをマージして main に取り込んだ後、次のフェーズの作業を Copilot エージェントに依頼するときは、  
**以下のように次のフェーズ番号を明示したメッセージ**を使ってください:

```
docs/REFACTORING.md の次のフェイズに進みます
```

このメッセージは毎回同じ文言で構いません。エージェントは `docs/REFACTORING.md` を読んで  
どのフェーズが完了済み（✅ マーク）かを確認し、次に着手すべきフェーズを自動的に判断します。

### 作業の流れのまとめ

```
1. 前のフェーズの PR を main にマージ
         ↓
2. Copilot エージェントに「docs/REFACTORING.md の次のフェイズに進みます」と依頼
         ↓
3. エージェントが次のフェーズを実装・PR 作成
         ↓
4. PR をレビュー → main にマージ
         ↓
5. 2 に戻る（全フェーズ完了まで繰り返し）
```
